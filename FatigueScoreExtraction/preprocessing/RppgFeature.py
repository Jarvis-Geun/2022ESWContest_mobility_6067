...
def read_text_file(ppgData, vidData):

    ppg_df = pd.read_csv(ppgData, delimiter=' ')
    video_df = pd.read_csv(vidData, delimiter=' ')
...
if __name__ == "__main__":
    ...
    for folder in tqdm(sorted(folders)):
        try:
            ...
            ppgData = [os.path.join(*[data_path, folder, folder + str(i) + ".txt"]) for i in range(1, 11)]
            vidData = [os.path.join(*[data_path, folder, folder + str(i) + "_frame_time.txt"]) for i in range(1, 11)]
            if folder == 'hwang':
                ppgData = ppgData[:-1]
                vidData = vidData[:-1]
            for data in tqdm(zip(ppgData, vidData)):
                ppg = read_text_file(data[0], data[1])
