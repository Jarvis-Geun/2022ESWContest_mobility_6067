...
from preprocessing import FacialFeature
from preprocessing import RppgFeature
from preprocessing import ConcatLabelData

from train import trainANN
from train import trainRegressor
...
if __name__ == '__main__':
    ...
    print("Raw Data Processing Part Start!\n")
    ...
    for folder in tqdm(sorted(folders)):
        try:
            ...
            for i in tqdm(range(1, 11)):
                ...
                time_fname = folder + str(i) + "_frame_time.txt"

                rppg_df = pd.DataFrame(columns=['HR', 'HR_std', 'SDNN', 'LF', 'HF', 'LF/HF'])
                facial_df = pd.DataFrame(columns=['PERCLOSE', 'Excessive Blink', 'Yawn'])

                ppg = RppgFeature.read_text_file(os.path.join(*[data_path, folder, ppg_fname]), os.path.join(*[data_path, folder, time_fname]))
                vid = FacialFeature.OneVidProcessing(os.path.join(*[data_path, folder, vid_fname]))
                label_results = ConcatLabelData.LabelData(os.path.join(*[data_path, folder, label_fname]), label_results)

                for i in range(6):
                    rppg_df.loc[i] = RppgFeature.mk_df_WESAD(ppg[3000 * i:3000 * (i + 1)])
    ...
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == '--pytorch':
            ...
            start = time.time()
            trainRegressor.RegressorTraining(x_train, y_train)
